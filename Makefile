...
build-debug build-release: build-%: build_%/CMakeCache.txt
	cmake --build build_$* -j $(NPROCS) --target matching

# Test
.PHONY: test-debug test-release
test-debug test-release: test-%: build-%
#	cmake --build build_$* -j $(NPROCS) --target matching_unittest
#	cmake --build build_$* -j $(NPROCS) --target matching_benchmark
...
service-start-debug service-start-release: service-start-%: build-%
	cmake --build build_$* -v --target start-matching
...
install-debug install-release: install-%: build-%
	cmake --install build_$* -v --component matching
...
--in-docker-start-debug --in-docker-start-release: --in-docker-start-%: install-%
	...
	/home/user/.local/bin/matching \
		--config /home/user/.local/etc/matching/static_config.yaml \
		--config_vars /home/user/.local/etc/matching/config_vars.docker.yaml

# Build and run service in docker environment
.PHONY: docker-start-service-debug docker-start-service-release
docker-start-service-debug docker-start-service-release: docker-start-service-%:
	$(DOCKER_COMPOSE) run -p 8080:8080 --rm matching-container make -- --in-docker-start-$*

# Start targets makefile in docker environment
.PHONY: docker-cmake-debug docker-build-debug docker-test-debug docker-clean-debug docker-install-debug docker-cmake-release docker-build-release docker-test-release docker-clean-release docker-install-release
docker-cmake-debug docker-build-debug docker-test-debug docker-clean-debug docker-install-debug docker-cmake-release docker-build-release docker-test-release docker-clean-release docker-install-release: docker-%:
	$(DOCKER_COMPOSE) run --rm matching-container make $*
